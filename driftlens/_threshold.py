...
class RandomSamplingThresholdEstimator(ThresholdEstimatorMethod):
    ...
    for i, (E_w, Y_w) in enumerate(zip(E_windows, Y_windows)):
        ...
        # Batch distance: Ensure enough samples for valid covariance
        if len(E_w) < 2:
            window_distances["per-batch"] = 0
            print(f"Warning: Window {i} has fewer than 2 samples ({len(E_w)}). Assigned default batch distance 0.")
        else:
            pca_batch = baseline.get_batch_PCA_model()
            if pca_batch is None:
                window_distances["per-batch"] = 0
                print(f"Warning: Batch PCA model is None for window {i}. Assigned default batch distance 0.")
            else:
                E_w_reduced = pca_batch.transform(E_w)
                batch_mean = fdd.get_mean(E_w_reduced)
                batch_cov = fdd.get_covariance(E_w_reduced)
                if batch_cov.size == 1:  # Scalar covariance (1 sample or degenerate case)
                    window_distances["per-batch"] = 0
                    print(f"Warning: Batch covariance for window {i} is scalar ({batch_cov}). Assigned default batch distance 0.")
                else:
        ...
        for label in self.label_list:
            ...
            if pca_model is None:
                ...
                if len(E_l_idxs) == 0:
                    ...
                    if cov_l.size == 1:  # Scalar covariance (degenerate case)
                        window_distances["per-label"][str(label)] = 0
                        print(f"Warning: Covariance for label {label} in window {i} is scalar ({cov_l}). Assigned default distance 0.")
                    else:
